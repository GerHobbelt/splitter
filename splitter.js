  ...
  $.fn.splitter = function (args) {
    ...
    return this.each(function () {
      ...
      function resplit(pos) {
        ...
        if ((opts.dockPane == A && pos < Math.max(A._min, bar._DA)) || (opts.dockPane ==
          B && pos > Math.min(pos, A._max, splitter._DA - bar._DA - B._min))) {
      ...
      function resize(size) {
        ...
        resplit(!isNaN(size) ? size : (!(opts.sizeRight || opts.sizeBottom) ? A[
          0][opts.pxSplit] : splitter._DA - B[0][opts.pxSplit] - bar._DA));
        setBarState(opts.barNormalClass);
      }
      ...
      opts.barStateClasses = [
        opts.barNormalClass, opts.barHoverClass, opts.barActiveClass, opts.barLimitClass
      ].join(' ');
      ...
      bar._DA = bar[0][opts.pxSplit];
      splitter._PBF = dimSum(splitter, 'border' + opts.side3 + 'Width',
        'border' + opts.side4 + 'Width');
      splitter._PBA = dimSum(splitter, 'border' + opts.side1 + 'Width',
      ...
      $.each([A, B], function () {
        ...
        this._max = opts['max' + this._pane] || dimSum(this, 'max-' + opts.split) ||
          9999;
        this._init = opts['size' + this._pane] === true ? parseInt($.css(this[0],
          opts.split), 10) : opts['size' + this._pane];
      });
      ...
      if (opts.resizeTo) {
        ...
        $(window).bind('resize' + opts.eventNamespace, function () {
          ...
          splitter.css('height', Math.max(eh - top - splitter._hadjust,
            splitter._hmin) + 'px');
      ...
      if (opts.dock) {
        splitter
          ...
          .bind('dock' + opts.eventNamespace, function () {
            var pw = A[0][opts.pxSplit], x = {};
            if (!pw) return;
            bar._pos = pw;
            x[opts.origin] = opts.dockPane == A ? 0 : splitter[0][opts.pxSplit] -
              splitter._PBA - bar[0][opts.pxSplit];
          ...
          .bind('undock' + opts.eventNamespace, function () {
            var pw = opts.dockPane[0][opts.pxSplit], x = {};
            if (pw) return;
            x[opts.origin] = bar._pos + 'px';
            bar
              .removeClass(opts.barDockedClass)
              .animate(x, opts.undockSpeed || opts.dockSpeed || 1, opts.undockEasing ||
                opts.dockEasing, function () {
                resplit(bar._pos);
                bar._pos = null;
              });
          });
      ...
      splitter
        .bind('destroy' + opts.eventNamespace, function () {
          ...
          splitter.removeClass(opts.splitterClass)
            .add(panes)
